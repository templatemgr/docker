...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202407161542-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  09-registry.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Jul 16, 2024 15:42 EDT
# @@File             :  09-registry.sh
# @@Description      :
...
[ -f "$CONF_DIR/env/registry.script.sh" ] && . "$CONF_DIR/env/registry.script.sh" # Generated by my dockermgr script
[ -f "$CONF_DIR/env/registry.sh" ] && . "$CONF_DIR/env/registry.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__post_execute() {
  ...
  (
    while :; do
      docker-registry garbage-collect /config/registry/config.yml --delete-untagged=true 2>/dev/stderr | tee -a -p "$LOG_DIR/garbage.txt"
      sleep 3600
    done |& tee -p -a "$LOG_DIR/garbage.txt" >/dev/null &
